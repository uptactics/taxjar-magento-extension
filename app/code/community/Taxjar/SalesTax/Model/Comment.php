...
class Taxjar_SalesTax_Model_Comment{
    public function getCommentText(){ //this method must exits. It returns the text for the comment
      ...
      if($lastUpdate){
        return "<br/><p>TaxJar has automatically added rates for " . $regionName . " and all other states configured in your TaxJar account. To view and manage these states <a href='https://app.taxjar.com/account#states'  target='_blank'>click here</a>.</p><br/><p>Your sales tax rates were last updated on: <ul><li>" . $lastUpdate . "</li></ul></p><p>If you would like to uninstall TaxJar, remove the API Token from the box above, then save the config.  This will remove all the rates.  You can then uninstall in the Magento Connect Manager.</p>";
      } else {
